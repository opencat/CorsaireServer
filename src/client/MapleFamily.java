...
public class MapleFamily {
    ...
    public static MapleFamily loadOrCreateFamily(MapleCharacter chr) {
        ...
        try {
            ...
            if (chr.getFamilyId() > 0) {
                ps = con.prepareStatement("SELECT * FROM family WHERE cid = ?");
                ...
                while (rs.next()) {
                    ...
                    wat.setCharacterId(rs.getInt("characterid"));
                ...
                ps = con.prepareStatement("INSERT INTO family (`characterid`, `name`, `familyid`) VALUES (?, ?, ?)");
    ...
    private void addMember(MapleCharacter chr, MapleCharacter snr) {
        if (!members.containsKey(chr.getId())) {
            ...
            try {
                ps = DatabaseConnection.getConnection().prepareStatement("INSERT INTO family (`characterid`, `familyid`, `senior`, `name`) VALUES (?, ?, ?, ?)");
                ps.setInt(1, chr.getId());
