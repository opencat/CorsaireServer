...
public class MapleReactor extends AbstractMapleMapObject {
    ...
    public MaplePacket makeDestroyData() {
        return ReactorFactory.destroyReactor(this);
    }
    ...
    public MaplePacket makeSpawnData() {
        return ReactorFactory.spawnReactor(this);
    }
                ...
                if (stats.getNextState(state) == -1) { //end of reactor
                    if (stats.getType(state) < 100) { //reactor broken
                        if (delay > 0) {
                            ...
                            map.broadcastMessage(ReactorFactory.triggerReactor(this, stance));
                        }
                    } else { //item-triggered on final step
                        map.broadcastMessage(ReactorFactory.triggerReactor(this, stance));
                    }
                    ReactorScriptManager.getInstance().start(c, this, "reactor/" + rid);
