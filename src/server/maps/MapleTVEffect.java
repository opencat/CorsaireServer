...
public class MapleTVEffect {
    ...
    private void broadcastTV(boolean active_) {
        ...
        try {
            if (active_) {
                wci.broadcastMessage(null, EffectFactory.enableTV().getBytes());
                wci.broadcastMessage(null, EffectFactory.sendTV(user, message, type <= 2 ? type : type - 3, partner).getBytes());
                int delay = 15000;
                if (type == 4) {
                    delay = 30000;
                } else if (type == 5) {
                ...
                wci.broadcastMessage(null, EffectFactory.removeTV().getBytes());
            }
